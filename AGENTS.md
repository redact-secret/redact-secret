...
Prepare an approved version through a reviewed pull request to `main`, qualify the exact merged commit, and dispatch publication from `main` after explicit release approval. The release workflow records `github.sha` as the immutable source revision and creates annotated `v{version}` at that exact commit only after all publication jobs and registry-install verification succeed. That peeled tag commit is the canonical released source identity. The workflow records a durable release manifest (source revision, conformance corpus identity, version, artifact set, registry state) for every run, including a failed one. Use `Reconcile Release` only with explicit authorization to repair a matching published version without republishing it; dispatch from `main`. Its repair window is any recorded release source commit that remains an ancestor of `main` (the current tip included), or that is named explicitly via the workflow's `source_commit` input — never a commit outside `main` history. See the [release runbook](docs/releasing.md#branching-model).
