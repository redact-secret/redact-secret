 ...
 * real result (`{ text, findings }`, `finding.action`). Self-contained
 * rather than imported from `examples/tracing-masking/fixtures/fake-scanner.mjs`
 * -- each example directory stands on its own -- but implements the same
 * four rules, by convention, so a reader who has seen one recognizes the
 * other:
 *
 ...
 *
 * Kept in sync by hand with `python/fake_scanner.py`; both implement the
 * same four rules so `fixtures/logging-redaction-cases.json` means the
