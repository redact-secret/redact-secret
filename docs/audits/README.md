...
| Database-SaaS credential assessment and Neon API key contract (#524) | [Neon `napi_` prefix provider-stated (T1), 64-byte body floor tool-corroborated (T2); connection-string overlap pinned; PlanetScale, CockroachDB Cloud and MongoDB Atlas ranked as follow-ups](evidence/524/README.md) | product judgement | kept in place |
| CI-provider credential ranking and Travis CI contract (#523) | [Travis CI chosen on two-tool corroboration; 22-byte `[A-Za-z0-9]` token gated on a same-line `travis` keyword, T2; CircleCI and Buildkite are follow-up candidates, GitHub Actions rejected](evidence/523/README.md) | product judgement | kept in place |
| Exact beta.8 candidate qualification (#731) | [Candidate identity, stable and new-family gate measurements, consumer paths, and the open baseline and unsupported-family questions; no publication authorized](evidence/731/README.md) | product judgement | kept in place |
| Beta.8 15-family contract freeze (#726) | [First-wave/second-wave selection, per-family grammar, qualification route, measured generic overlap, cost and FP/FN boundary](evidence/726/README.md) | product judgement | final record; benchmark measurements remain in `redact-secret-benchmarks`, linked by pinned commit |
| T1 provider evidence for 8 families: anthropic, linear, notion, new-relic user, grafana glc/glsa, azure-devops, google (#642) | [All 8 prefixes found on provider domains and all 8 product detectors match; the pinned-run "Done when" depends on benchmarks#112](evidence/642/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
| Epic A portfolio, second stabilization batch and qualification (#575, #707, #708) | [10 efficiency + 5 market-value portfolio; 7 more families T1 and `stable`; Entra leading-dash and Docker 27-byte OAT fixes; 49 stable, 0 regressions, 0 leaked spans in candidate mode](evidence/575/README.md) | product judgement | kept in place — benchmark measurement lives in `redact-secret-benchmarks` evidence/575, linked |
| Beta.7 candidate qualification (#584) | [Candidate `6a2dca0` at benchmarks `28fc818`: 51 stable (+17 from 34), 0 regressions, 0 leaked T1/T2 spans; unresolved differential items in 4 provisional families open](evidence/584/README.md) | product judgement | kept in place — final record; benchmark run evidence stays in `redact-secret-benchmarks` |
| T1 provider evidence for `atlassian:api-token` (#643) | [Not found (exhaustive, 2026-09-23): no provider-documented shape; `ATAT` prefix stated only by an Atlassian-staff community answer, pending maintainer ruling](evidence/643/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
| T1 provider evidence for `datadog:api-key` (#644) | [Found (length + marker): docs.datadoghq.com full_spec_v1.json fixes ApiKey.key at 32/32 and names DD-API-KEY / DD_API_KEY; alphabet stays tool-corroborated](evidence/644/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
| T1 provider evidence for `datadog:application-key` (#645) | [Found: docs.datadoghq.com states the `ddapp_` prefix; body length and alphabet are backed only by Datadog code and AWS; the docs-served v1 OpenAPI gives legacy length 40 only](evidence/645/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
...
| T1 provider evidence for `generic:connection-string-password` (#651) | [Not found (exhaustive, 2026-09-23): the RFCs and vendors give only the userinfo delimiter and alphabet, no prefix, length or marker; a PlanetScale `pscale_pw_` T1 source exists but is a separate subfamily](evidence/651/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
...
| T1 provider evidence for `huggingface:api-token` (#654) | [Found: huggingface.co SDK reference types the token `hf_${string}`; OpenAPI example fixes the 34-char body on example strength only; alphabet and `api_org_` not provider-stated](evidence/654/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
| T1 provider evidence for `microsoft-entra:application-client-secret` (#655) | [Found by example only: Graph PowerShell reference shows 3 + `8Q~` + 34 (40); no prose grammar; contract rejects a leading `-`](evidence/655/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
...
| T1 provider evidence for `sentry:organization-auth-token` (#658) | [Not found on a Sentry domain (exhaustive, 2026-09-23); only candidate is Sentry's github.com-hosted RFC 0091 (`sntrys_` + two-`_` structure), pending maintainer ruling](evidence/658/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
| T1 provider evidence for `sentry:user-auth-token` (#659) | [Prefix `sntryu_` appears only as a placeholder on skills.sentry.dev (page mislabels it "org auth token"); length/alphabet provider-code on github.com only — T1 sufficiency is a maintainer call](evidence/659/README.md) | product judgement | kept in place — final record; research passes stay in issue comments, linked |
...
| Shadow evidence scorer qualification (#772) | [Cross-runtime determinism on Linux, macOS, Windows and wasm32, whole-input vs incremental parity, hostile worst cases, and the size fix that keeps the scorer out of shipped builds](evidence/772/README.md) | product judgement | kept in place — final record; performance and size budget verdicts live in redact-secret-benchmarks `evidence/772/` |
