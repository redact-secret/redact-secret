...
Reviewed on 2026-09-20 for release tracking issue
...
`main` revision `1320ff73f4900fe2057751818e2a93e80c51b4c2`. The exact frozen
...
change. The readiness
review that preceded it, and the fixes it required in two passes, are in
[beta.5 release readiness review](beta5-release-readiness-review.md).
...
No detector id, finding type, confidence, or specificity changed for any
existing detector; the two provider-token additions below reuse their
existing family's id and type.
...
Two provider-token coverage variants, deferred out of the seven-family
precision fix as known false negatives, are adopted at the same reviewed
grammar as their existing family: Cloudflare's `cfat_` (account token, #481)
over `cfut_`'s exact body and checksum shape, and Hugging Face's `api_org_`
(organization token, #485) over `hf_`'s exact body shape, re-tiered from T0 to
T2 on the same dual-tool evidence `hf_` itself already relies on. Neither adds
a detector id or finding type. Cloudflare's third documented prefix, `cfk_`,
stays unsupported: no tool corroborates its checksum shape, so freezing it
would be a guess rather than a reviewed contract (split into #486).

Four false-positive fixes narrow detection without adding or removing a
public identifier: `generic-token` no longer reports a closed or
argument-truncated source-code call expression assigned to a credential-named
variable (#467 — the truncated case previously produced *corrupted* redacted
output by splitting a redaction mid-expression, not merely an unwanted
finding); `bearer-token` now excludes repeated-character filler and
whole-value placeholder vocabulary the way `generic-token` already does for
`Basic`/`Token` (#468); `connection-string` now excludes an
environment-variable-reference password the way `generic-token` already does
for a contextual assignment, sharing the predicates through `super::text`
(#469); and `jwt` no longer reports a legacy-format Supabase anonymous key
identified by its decoded `iss`/`role` claims, while `service_role` and every
other claim combination is still reported and the signature is still not
verified (#472). Each is recorded in its own dated decision record and stays
within the existing detector's id, type, confidence, and specificity.
...
qualification gaps found on 2026-09-19 and fixed in PR #465) is closed. A
second readiness pass on 2026-09-20, after #467–#472, #481, and #485 merged
to `main`, found the same class of gap for that work — a missing changelog
entry and, for the two coverage additions, a missing decision record — and
fixed both (see the readiness review's second section). The non-blocking
items and their dispositions are listed in the readiness review.
